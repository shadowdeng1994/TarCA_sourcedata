...
def calc_node(node, n, t, tree_struct, diff_time, diff_mode_probs, life_time, tar_t, p_death, p_nd, cor_n, diff1, diff2):
    if node[0] == '-':
        #t_life = tar_t - t
    ...
    pop_i = calc_node(node = tree_struct[node][0], n = n_i, t = t + life_time[node] * (double_round + 1),
                    tree_struct = tree_struct, diff_time = diff_time, cor_n = cor_n_i, diff1 = diff_mode_probs[node][0], diff2 = diff_mode_probs[node][2],
    ...
    pop_j = calc_node(node = tree_struct[node][1], n = n_j, t = t + life_time[node] * (double_round + 1),
                    tree_struct = tree_struct, diff_time = diff_time, cor_n = cor_n_j, diff1 = diff_mode_probs[node][1], diff2 = diff_mode_probs[node][2],
                    diff_mode_probs = diff_mode_probs, life_time = life_time, tar_t = tar_t, p_death = p_death, p_nd = p_nd)
    rtn = pop_i
    #rtn.update(pop_i)
    rtn.update(pop_j)
...
def calc_pop(info):
    ...
    node_pop = calc_node(node = info['root_id'], n = n0, t = 0, tree_struct = tree_struct, diff_time = diff_time,
                        ...
                        p_death = p_death, p_nd = p_nd, cor_n = 0 ,diff1=0, diff2=0)
    return node_pop


if __name__ == "__main__":
    read_path = 'D:\\SYSU\\helab\\TCA\\QFM\\supplementary_data_1_fate_map_panel/'
    write_path = 'D:\\SYSU\\helab\\TCA\\QFM\\fraction\\'
    ...
    for path_json in glob.glob(read_path + "fate_map*.json"):
        ...
        file_out = open(write_path + "fraction_" + path_json.split('\\')[-1].split('.')[0] + ".csv", 'w')
        for key in pop_size.keys():
            file_out.write('"' + key + '",' + str(pop_size[key][0])  + ',' + str(pop_size[key][1]) + '\n')
